...
FILTER_CLASSES = ['text_free']  # 只保留这些类别的检测结果，例如：'bubble', 'text_bubble', 'text_free'
...
for idx, folder_path in enumerate(folder_list, start=1):
    ...
    for img_path in tqdm(image_files, desc=f"推理 {folder_name[:30]}", total=total_images, unit="img"):
        ...
        # 统计当前图片检测到的目标数，如为 0 则打印调试信息
        total_objs = 0
        for res in results:
            total_objs += len(res.get("scores", []))
        if total_objs == 0:
            print(f"[DEBUG] 图片 {img_name} 未检测到物体。")
        ...
        if result_lines:
            ...
            print(f"[DEBUG] 图片 {img_name} 的 TXT 内容为空。")

        # 保存推理后带检测框的图片至脚本目录下的 results 文件夹
        # 计算相对于“本篇”目录的相对路径以便保留原有目录结构
