...
model = YOLO(r'J:\G\Desktop\yolo模型存放\YOLOV11X40901天_27595张_85轮\YOLOV11X40901天_27595张_85轮best.pt', task='detect')  # 请修改为你的模型路径
    ...
    try:
        ...
        for i, image_path in enumerate(image_paths):
            ...
            if GENERATE_MASK:
                img = result.orig_img
                # 创建4通道图像，实现透明背景
                mask_color_map = np.zeros((img.shape[0], img.shape[1], 4), dtype=np.uint8)
                custom_color = (238, 255, 0, 255)  # GRB 颜色，(蓝色, 绿色, 红色, alpha)
                for box in result.boxes:
                    cls = int(box.cls[0])
                    class_name = model.names[cls]  # 获取类别名称
                    if ENABLE_FILTER and class_name not in FILTER_CLASSES:
