...
# ============== 新增：角度范围模式开关与配置 ==============
# 是否启用“指定范围模式” True  False
USE_ANGLE_RANGE = True
# 指定角度范围/列表（顺时针），示例："300-358" 或 "30,45,60" 或 "10-20, 90, 120-125"
ANGLE_RANGE_SPEC = "300-358"
# ======================================================
...
def _parse_angle_spec(spec):
    """
    解析像 "300-358" 或 "30,45,60" 或 "10-20, 90, 120-125" 的字符串，
    返回去重且升序的整数角度列表（范围均为 0..359 内取模）。
    """
...
def _build_angle_list():
    """
    根据 USE_ANGLE_RANGE 与 ANGLE_RANGE_SPEC 生成最终角度列表（顺时针角度），
    并应用 EXCLUDED_ANGLES 排除规则；同时默认不包含 0/360，除非显式指定范围含 0。
    """
    excluded = {int(a) % 360 for a in EXCLUDED_ANGLES}

    if USE_ANGLE_RANGE:
        base = _parse_angle_spec(ANGLE_RANGE_SPEC)
        # 默认行为：如果用户范围里包含 0，就保留 0；否则不自动加 0。
        angles = [a for a in base if a not in excluded]
        mode_note = "开启指定范围模式"
    else:
        # 原逻辑：1..359
        angles = [a for a in range(1, 360) if a not in excluded]
        mode_note = "使用默认角度模式（1..359，应用排除列表）"
...
def process_folder_all_angles(folder_path):
    ...
    if not angles:
        print_safe("角度集合为空，请检查 USE_ANGLE_RANGE 与 ANGLE_RANGE_SPEC 或 EXCLUDED_ANGLES。")
        return
