...
SKIP_BIAOQIAN = True                   # 控制是否跳过已有 biaoqianTXT 文件夹 开=True 关=False
GENERATE_MASK = True                    # 控制是否生成掩膜图 开=True 关=False
...
def process_image_folder(folder_path, current_index, total_folders):
    try:
        ...
        # 设置矩形框的颜色：透明度255的黄色（GRB）
        custom_color = (238, 255, 0, 255)  # GRB颜色，(蓝色, 绿色, 红色, alpha)
        ...
        for batch_start in range(0, total_images, BATCH_SIZE):
            ...
            for result, image_path in zip(results, batch_paths):
                ...
                if GENERATE_MASK:
                    img = result.orig_img
                    mask_color_map = np.zeros((img.shape[0], img.shape[1], 4), dtype=np.uint8)  # 4通道图像，包含透明度
                    for box in result.boxes:  # 迭代每个检测框
                        ...
                        if cls in ADJUST_PARAMS:  # 只处理指定类别
                            ...
                            # 使用自定义颜色绘制矩形框，最后一个参数为透明度255（完全不透明）
                            cv2.rectangle(mask_color_map, (x1, y1), (x2, y2), custom_color, -1)  # 填充矩形框区域
                        else:
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        for index, folder_path in enumerate(paths, start=1):
            process_image_folder(folder_path, index, total_folders)
    else:
        print("请拖拽文件夹到脚本上运行")
