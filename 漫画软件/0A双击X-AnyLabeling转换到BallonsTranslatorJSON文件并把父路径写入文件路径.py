...
def convert_xal_shape_to_balloon_obj(shape):
    ...
    obj = {
        ...
        "merged": False, "text": [""], "translation": "", "rich_text": "",
...
def main():
    ...
    json_files = sorted([f for f in os.listdir(work_dir) if f.lower().endswith('.json')])
    ...
    processed_file_count = 0  # ***新增***: 用于统计最终成功转换的文件数
    ...
    for filename in json_files:
        ...
        page_objects = []
        filtered_count = 0  # 用于统计每个文件过滤掉的标签数
        ...
        if page_objects:
            ...
            processed_file_count += 1 # ***新增***: 只有成功转换的文件才计数
